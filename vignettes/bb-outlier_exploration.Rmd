...
Analyse d'un fichier type '02_simplified.xlsx'

## km_sortie (3.1.14)

L'idee est de sortir le premier et le dernier decile pour qu'ils puissent être analysés.
...
data_km %>% 
  group_by(categorie) %>% 
  summarize(p10 = quantile(km_sortie, 0.1),
...
data_km %>% 
  group_by(categorie) %>% 
  filter(km_sortie < quantile(km_sortie, 0.1))
### Last decile
data_km %>% 
  group_by(categorie) %>% 
  filter(km_sortie > quantile(km_sortie, 0.9))
...
Meme type d'approche avec la detection du permier et dernier decile. Par contre cette fois le groupement peut se faire par categorie et par mode_heb_regroupe.

Je teste les 2 types de regroupements:

### Regroupement par categorie

```{r}
data_dms <- eva_data$enquete %>% 
  select(id_quest, dms, 
         categorie, mode_heb_regroupe) %>% 
  filter(!is.na(dms)) %>% 
  group_by(categorie)

# Verification de quantiles
data_dms %>% 
  summarize(p10 = quantile(dms, 0.1),
...
### First approach with percentile
data_dms %>% 
  filter(dms < quantile(dms, 0.1))

data_dms %>% 
  filter(dms > quantile(dms, 0.9))
...
```{r}
data_revenu <- eva_data$enquete %>% 
  select(id_quest, revenu) %>% 
  filter(!is.na(revenu))

data_revenu %>% 
  summarize(p10 = quantile(revenu, 0.1),
...
## First approach with percentile
data_revenu %>% 
  filter(revenu < quantile(revenu, 0.1))

data_revenu %>% 
  filter(revenu > quantile(revenu, 0.9))
...
data_depense %>% 
  group_by(depense) %>% 
  summarise(p10 = quantile(valeur, 0.1),
...
## First approach with percentile
data_depense %>% 
  filter(valeur < quantile(valeur, 0.1))

data_depense %>% 
  filter(valeur > quantile(valeur, 0.9))
